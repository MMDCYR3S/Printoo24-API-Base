    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex justify-between items-center border-b border-slate-100 pb-4">
        <h1 className="text-2xl font-black text-slate-800 flex items-center gap-2">
          <Package className="text-primary" /> سفارش‌های من
        </h1>
        <div className="badge badge-lg badge-primary badge-outline font-bold">{orders.length} سفارش</div>
      </div>
      ...
      <div className="grid gap-4">
        {orders.length === 0 ? (
          <div className="text-center py-16 bg-slate-50 rounded-3xl border border-dashed border-slate-200">
             ...
             <p className="text-slate-500 font-bold">هنوز سفارشی ثبت نکرده‌اید.</p>
             <Link to="/shop" className="btn btn-primary btn-sm mt-4">ثبت اولین سفارش</Link>
          </div>
        ) : (
          orders.map((order) => (
            <div key={order.id} className="bg-white p-5 rounded-3xl border border-slate-100 shadow-sm hover:shadow-md transition-all group relative overflow-hidden">
               ...
               <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-4 pl-4">
                 <div className="flex gap-4 items-center">
                   ...
                   <div>
                     <h3 className="font-bold text-slate-800 text-lg">{order.type_display || 'سفارش چاپ'}</h3>
               ...
               <div className="flex flex-col md:flex-row items-start md:items-center justify-between pt-4 border-t border-slate-50 gap-4 pl-4">
                 <div className="flex items-center gap-2 text-xs font-medium text-slate-500 bg-slate-50 px-3 py-2 rounded-xl">
                   ...
                   <span className="truncate max-w-[250px]">{order.address || 'آدرس ثبت نشده'}</span>
                 </div>
                 
                 <div className="flex items-center gap-4 w-full md:w-auto justify-between">
                   <div className="text-right">
                      <span className="block text-[10px] text-slate-400">مبلغ کل</span>
                      <span className="font-black text-lg text-slate-700 dir-ltr">
                        {new Intl.NumberFormat('fa-IQ').format(order.total_price)} <span className="text-xs font-medium text-slate-400">IQD</span>
                      </span>
                   </div>
