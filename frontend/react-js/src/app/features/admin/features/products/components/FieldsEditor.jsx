...
const FieldsEditor = () => {
    ...
    const handleDragEnd = (event) => {
        ...
        if (active.id !== over.id) {
            const oldIndex = fields.findIndex((f) => (f.id || f.temp_id) === active.id);
        ...
        <div className="bg-white/70 backdrop-blur-xl shadow-2xl shadow-slate-200/50 border border-white p-8 rounded-[2rem]">
            ...
            <div className="flex justify-between items-start mb-8">
                <div className="flex items-start gap-5">
                    ...
                    <div>
                        <h3 className="font-extrabold text-slate-800 text-2xl tracking-tight">ساختار فرم (فیلدها)</h3>
                        <p className="text-sm text-slate-500 mt-2 font-medium">فیلدهای محصول و شروط نمایش آن‌ها را مدیریت کنید.</p>
                    </div>
                </div>
                <button onClick={handleAddField} type="button" className="btn btn-primary btn-sm rounded-full shadow-lg shadow-primary/20 hover:scale-105 px-6">
            ...
            <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
                <SortableContext items={fields.map(f => f.id || f.temp_id)} strategy={verticalListSortingStrategy}>
            ...
            {fields.length === 0 && (
                <div className="flex flex-col items-center justify-center text-slate-400 border-2 border-dashed border-slate-200 rounded-[2rem] bg-slate-50/50 py-16 mt-4">
                    ...
                    <h4 className="font-extrabold text-slate-600 text-lg">هنوز فیلدی اضافه نکرده‌اید</h4>
                    <button onClick={handleAddField} type="button" className="btn btn-outline btn-primary rounded-full px-8 mt-6">ساخت اولین فیلد</button>
                </div>
            )}
        </div>
    );
