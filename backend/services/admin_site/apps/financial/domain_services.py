...
from core.models import User, Order, Invoice, Quotation
...
class FinancialService:
    ...
    def issue_invoice_from_order(self, order: Order, user: User = None) -> Invoice:
        ...
        # ===== بررسی وجود فاکتور ===== #
        existing_invoice = Invoice.objects.get_by_invoice_number(invoice_num)
        ...
        try:
            invoice = Invoice.objects.create_invoice(invoice_data)
    ...
    def recalculate_invoice_totals(self, invoice: Invoice, user: User = None) -> Invoice:
        ...
        """
        if hasattr(invoice, 'allows_editing') and not invoice.allows_editing:
             raise ValidationError("فاکتور جهت محاسبه مجدد قفل است.")
        if invoice.status == Invoice.Status.FINALIZE:
             raise ValidationError("فاکتور نهایی شده جهت محاسبه مجدد قفل است.")
    ...
    def convert_quotation_to_invoice(self, quotation_id: int, user: User, order_id: int) -> Invoice:
        ...
        # ===== ایجاد فاکتور ===== #
        invoice = Invoice.objects.create_invoice({
            "order": order,
    ...
    def get_quotation_by_order(self, order_id: int):
        ...
        return Quotation.objects.get_quotation_by_order(order_id)
