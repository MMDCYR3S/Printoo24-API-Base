...
class QuotationRepository(BaseRepository[Quotation]):
    ...
    def get_quotation_by_order(self, order_id: int) -> Optional[Quotation]:
        """
        یافتن پیش‌فاکتور بر اساس شناسه سفارش.
        نکته: از select_related برای جلوگیری از کوئری اضافی روی اردر استفاده می‌کنیم.
        """
        return self.model.objects.select_related(
            'converted_order', 
